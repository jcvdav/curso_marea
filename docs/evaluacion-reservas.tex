...
La interpretación de estos coeficientes puede ser un poco compleja, pero
el siguiente ejemplo puede ayudarnos a aclarar dudas. Ajustando el
modelo lineal a los datos de la Figura \ref{fig:baci-multi}, obtenemos
los coeficientes mostrados en la Tabla \ref{tab:reg}.
...
el modelo ajustado, en este caso con un valor de \(R^2 = 0.87\).
...
decir, la diferencia entre las líneas rojas. Este término es el efecto
de la reserva.

Aunque esto puede parecer complicado, la realidad es que puede
implementarse con una línea de código como
\texttt{lm(indicador\ \textasciitilde{}\ post\ *\ zona,\ data\ =\ datos)}.
Mejor aún, MAREA se encarga de generar las variables \(Post\) y \(Zona\)
y de ajustar los coeficientes. Otra
